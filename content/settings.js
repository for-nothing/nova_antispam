window.addEventListener("load",function(){ 
	...
	sync_book.addEventListener("click",function(){
	...
	accManagement.addEventListener("click",function(){
		accIdNum = 0;
		enbl_accnts = [];
		acc_mngmnt();
	}, false);
...
function acc_mngmnt(){
	...
	if(accIdNum < identities.length){
		...
		var result = prompts.confirm(null, "управление аккаунтами", "На вашем клиенте thunderbird обнаружен аккаунт " + identity.email + ".Желаете использовать на нём nova_antispam?");
		if(result){
